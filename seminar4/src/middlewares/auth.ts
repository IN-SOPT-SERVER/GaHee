...
export default async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!token) return res.status(sc.UNAUTHORIZED).send(fail(sc.UNAUTHORIZED, rm.EMPTY_TOKEN)); // 토큰이 없음 : 인증된 사람이 아님 
  ...
  try {
    ...
    //? decode한 후 담겨있는 userId를 꺼내옴 [JwtPayload: Jwt가 제공하는 타입]
